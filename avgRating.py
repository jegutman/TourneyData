    ...
    for line in data:
        ...
        if datePrefix in line:
            for i in line.split('</tr>'):
                ...
                if eventId in EXCLUDED_TOURNAMENTS:
                    continue
                #if city == 'AUSTIN' and state == 'TX':
                #    playerInfo = findHighestRated2(eventId)
                #    if playerInfo[2] > 2150:
                #        print i, playerInfo
                #if state not in ('AZ', 'TN', 'OH'):
                if not statecode:
                    pass
                    #if state not in ('UT', 'NV', 'RI', 'IA', 'OR'):

                    #if state not in (statecode):
                        #continue
                elif state != statecode:
                    continue
                #if int(players) < 20:
                #    continue
                print state,
                ...
                #if int(i[-2]) < 30:
                #    continue
                if avgRating > 1900 and int(players) > 20:
                    #print avgRating, state, eventName, eventId
                    pass
                #print i, avgRating
