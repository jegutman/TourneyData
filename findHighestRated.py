...
def getRating(uscfId):
    filename = '~jgutman/Dropbox/TourneyDataRepo/uscfRatings'
    filename = os.path.expanduser(filename)
    tmpfile = '~jgutman/Dropbox/TourneyDataRepo/onerating'
...
def getPlayerHistory(uscfId):
    ...
    for page in range(1,10):
        filename = '~jgutman/Dropbox/TourneyDataRepo/playerInfo/playerInfo-%(uscfId)s.%(page)s' % locals()
        filename = os.path.expanduser(filename)
...
def getPlayerTourneyHistory(uscfId, tournamentId, section):
    ...
    while len(section) < 3:
        section = '0' + section
    filename = '~jgutman/Dropbox/TourneyDataRepo/playerInfo/%(tournamentId)s-%(section)s-%(uscfId)s' % locals()
    filename = os.path.expanduser(filename)
    try:
        data = open(filename)
    except:
